...
function renderOverviewView(m) {
  ...
  const CONV_PRIORITY = ['purchase','offsite_conversion.fb_pixel_purchase','onsite_conversion.purchase','lead','offsite_conversion.fb_pixel_lead','complete_registration','onsite_conversion.messaging_conversation_started_7d','onsite_conversion.total_messaging_connection','onsite_conversion.messaging_first_reply'];
  const convAction = m?.actions?.find(a => CONV_PRIORITY.indexOf(a.action_type) !== -1 && CONV_PRIORITY.indexOf(a.action_type) === Math.min(...m.actions.filter(x => CONV_PRIORITY.includes(x.action_type)).map(x => CONV_PRIORITY.indexOf(x.action_type))));
  const conv  = m ? (convAction ? convAction.value : '—') : '—';
  return `
